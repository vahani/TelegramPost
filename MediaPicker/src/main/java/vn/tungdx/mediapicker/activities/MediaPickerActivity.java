...
import vn.tungdx.mediapicker.activities.MediaPickerFragment.ClickStatus;
...
public class MediaPickerActivity extends AppCompatActivity implements
		MediaSelectedListener, CropListener,
		FragmentManager.OnBackStackChangedListener, FragmentHost, ClickStatus {
	 ...
	 * {@link Activity#onActivityResult(int, int, Intent)} of activity or
	 * fragment that open media picker. Always return {@link ArrayList} of
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		if (android.os.Build.VERSION.SDK_INT >= android.os.Build.VERSION_CODES.LOLLIPOP) {
			...
			TaskDescription taskDesc = new TaskDescription("", bm,
					getResources().getColor(R.color.colorPrimary));
			setTaskDescription(taskDesc);
			// TaskDescription
		}
		// not support change orientation right now (because out of
		...
		if (getActivePage() == null) {
			...
			getSupportFragmentManager().beginTransaction()
					.replace(R.id.container, new BucketImageFragment())
					.commit();
		}
	...
	public boolean onCreateOptionsMenu(Menu menu) {
		...
		mDone.setTitle(getString(R.string.letsOK));
		syncActionbar();
	...
	private int checkValidVideo(Uri videoUri) {
		...
		long duration = MediaUtils.getDuration(getApplicationContext(),
				MediaUtils.getRealVideoPathFromURI(getContentResolver(),
						videoUri));
		if (duration == 0) {
	 ...
	 * {@link Activity#onActivityResult(int, int, Intent)} of activity or
	 * fragment that open media picker.
	 *
	 * @param intent
	 *            In {@link Activity#onActivityResult(int, int, Intent)} method
	 *            of activity or fragment that open media picker.
	...
	@Override
	public void LetsOK(boolean done) {
		if (done) {
			isDone();
		}
	}
	...
	private void isDone() {
		...
		if (isPhoto) {
			if (mMediaOptions.isCropped()
				...
				// get first item in list (pos=0) because can only crop 1
				// image at same time.
