...
public class VideoFragment extends Fragment implements OnScrollListener {
	...
	public void initVideos() {
		...
		try {
			...
			mCursor = getActivity().getContentResolver().query(
					MEDIA_EXTERNAL_CONTENT_URI, proj, null, null,
					orderBy + " DESC");
			setAdapter();
		} catch (Exception e) {
			e.printStackTrace();
	...
	private void setAdapter() {
		...
		if (count > 0) {
			...
			mVideoAdapter = new GridViewAdapter(getActivity(), 0,
					mGalleryModelList, true);
		...
		mVideoGridView
				.setOnItemLongClickListener(new OnItemLongClickListener() {
					...
					public boolean onItemLongClick(AdapterView<?> parent,
						...
						GridViewAdapter adapter = (GridViewAdapter) parent
								.getAdapter();
			...
			public void onItemClick(AdapterView<?> parent, View view,
				...
				if (!galleryModel.status) {
					long size = MediaChooserConstants.ChekcMediaFileSize(
							new File(galleryModel.url.toString()), true);
					if (size != 0) {
						Toast.makeText(
								getActivity(),
